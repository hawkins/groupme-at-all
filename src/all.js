...
class AllBot {
  ...
  run() {
    // TODO: Extract these anonymous functions into their own spot somewhere

    // Get ID command
    this.robot.hear(/get id (.+)/i, res => {
    ...
    this.robot.hear(/blacklist (.+)/i, res => {
      ...
      if (!user)
        return res.send(`Could not find a user with the name ${target}`);
